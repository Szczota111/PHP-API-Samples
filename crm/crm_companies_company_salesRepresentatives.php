<?php

require_once __DIR__ . '/../vendor/autoload.php';
require_once __DIR__ . '/../api.php';

$api = new Api("https://demo.contractors.es", "admin", "admin", "en");

try {
    $company = $api->first('/api/crm/companies');
    if (!$company || !isset($company['id'])) {
        throw new RuntimeException('Unable to resolve a company for listing sales representatives.');
    }

    $endpoint = '/api/crm/companies/{company}/salesRepresentatives';
    $endpoint = strtr($endpoint, [
        '{company}' => $company['id'],
    ]);

    // Fetch list of sales representatives (query params support with_trashed/only_trashed)
    $response = $api->get($endpoint);
    $body = json_decode($response->getBody()->getContents(), true);

    if (empty($body['data'])) {
        $contacts = $api->getAll('/api/crm/contacts?limit=5');
        $contactIds = array_column($contacts ?? [], 'id');
        if (empty($contactIds)) {
            throw new RuntimeException('Unable to locate contacts to attach as sales representatives.');
        }

        $attachEndpoint = strtr('/api/crm/companies/{company}/salesRepresentatives/attach', [
            '{company}' => $company['id'],
        ]);

        $api->post($attachEndpoint, [
            'resources' => array_slice($contactIds, 0, 2),
        ]);

        $response = $api->get($endpoint);
        $body = json_decode($response->getBody()->getContents(), true);
    }
    echo json_encode($body, JSON_PRETTY_PRINT) . PHP_EOL;
} catch (Exception $e) {
    echo 'Error: ' . $e->getMessage() . PHP_EOL;
    exit(1);
}
