...
try {
    ...
    if (!$meeting || !isset($meeting['id'])) {
        throw new RuntimeException('Nie znaleziono żadnego spotkania.');
    }
    ...
    if (empty($employees)) {
        ...
        if (empty($contactIds)) {
            throw new RuntimeException('Brak kontaktów do podpięcia jako pracownicy.');
        }
    ...
    $endpoint = '/api/crm/meetings/{meeting}/employees';
