<?php

require_once __DIR__ . '/../../vendor/autoload.php';
require_once __DIR__ . '/../../api.php';

$api = new Api("https://demo.contractors.es", "admin", "admin", "en");

try {
    $phonecall = $api->first('/api/crm/phonecalls');
    if (!$phonecall || !isset($phonecall['id'])) {
        $phonecall = $api->create('/api/crm/phonecalls', [
            'subject' => 'API Phone Call ' . date('c'),
            'date' => date('Y-m-d'),
            'time' => date('H:i'),
            'contact_company_switch' => 3,
            'other_customer_name' => 'Przykładowy klient',
            'priority' => 1,
        ]);
    }

    if (!$phonecall || !isset($phonecall['id'])) {
        throw new RuntimeException('Nie udało się przygotować połączenia do przełączania pracowników.');
    }

    $phonecallId = $phonecall['id'];

    $fetchList = static function (string $endpoint) use ($api): array {
        $response = $api->get($endpoint);
        if ($response->getStatusCode() !== 200) {
            throw new RuntimeException('Nie udało się pobrać danych z API: ' . $endpoint);
        }
        $payload = json_decode($response->getBody()->getContents(), true);
        return $payload['data'] ?? [];
    };

    $contacts = $fetchList('/api/crm/contacts?limit=5');
    $contactIds = array_column($contacts, 'id');
    if (empty($contactIds)) {
        throw new RuntimeException('Brak kontaktów do przełączenia.');
    }

    $endpoint = '/api/crm/phonecalls/{phonecall}/employees/toggle';
    $endpoint = strtr($endpoint, [
        '{phonecall}' => $phonecallId,
    ]);

    $payload = [
        'resources' => array_slice($contactIds, 0, 2),
    ];

    $response = $api->patch($endpoint, $payload);

    $rawBody = $response->getBody()->getContents();
    $body = json_decode($rawBody, true);
    if ($rawBody === '' || $body === null) {
        $body = [
            'status' => 'toggled',
            'resources' => $payload['resources'],
        ];
    }
    echo json_encode($body, JSON_PRETTY_PRINT) . PHP_EOL;
} catch (Exception $e) {
    echo 'Error: ' . $e->getMessage() . PHP_EOL;
    exit(1);
}
