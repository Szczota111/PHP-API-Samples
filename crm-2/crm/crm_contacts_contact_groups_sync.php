...
try {
    ...
    if (!$contact || !isset($contact['id'])) {
        throw new RuntimeException('Unable to resolve a contact for syncing groups.');
    }

    $groups = $api->getAll('/api/crm/contact-groups?limit=5');
    $groupIds = array_column($groups ?? [], 'id');
    if (empty($groupIds)) {
        throw new RuntimeException('No contact groups available to sync.');
    }
    ...
    // Sync contact groups by replacing relations with provided list
    $payload = [
