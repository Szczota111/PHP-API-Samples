...
try {
    ...
    $employees = $fetchList("/api/crm/tasks/{$taskId}/employees?limit=5");
    if (empty($employees)) {
    ...
    if (empty($employees)) {
        throw new RuntimeException('Nie znaleziono pracownika powiązanego z zadaniem.');
    }

    $employeeId = $employees[0]['id'];

    $endpoint = '/api/crm/tasks/{task}/employees/{employee}';
